...
function XBlockValidationFactory(validationMessages, hasEditingUrl, isRoot, isUnit, validationEle) {
  ...
  model = new __WEBPACK_IMPORTED_MODULE_1_js_models_xblock_validation__(response, {
    parse: true
  });

  if (!model.get('empty')) {
    new __WEBPACK_IMPORTED_MODULE_0_js_views_xblock_validation__({
      el: validationEle,
      model: model,
      root: isRoot
    }).render();
  ...
  var XBlockValidationModel = Backbone.Model.extend({
    ...
    parse: function parse(response) {
      if (!response.empty) {
        ...
        if (!summary.type) {
          summary.type = this.WARNING; // Possible types are ERROR, WARNING, and NOT_CONFIGURED. NOT_CONFIGURED is treated as a warning.
