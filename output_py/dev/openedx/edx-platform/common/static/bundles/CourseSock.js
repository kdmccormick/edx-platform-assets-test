...
/* globals Logger */
var CourseSock = /*#__PURE__*/_createClass( // eslint-disable-line import/prefer-default-export
function CourseSock() {
  ...
  var pageLocation = window.location.href.indexOf('courseware') > -1 ? 'Course Content Page' : 'Home Page'; // Behavior to fix button to bottom of screen on scroll

  var fixUpgradeButton = function fixUpgradeButton() {
    if (!$upgradeToVerifiedButton.is(':visible')) return; // Grab the current scroll location

    var documentBottom = $(window).scrollTop() + $(window).height(); // Establish a sliding window in which the button is fixed

    var startFixed = $verificationSock.offset().top + 320;
    var endFixed = startFixed + $verificationSock.height() - 220; // Ensure update button stays in sock even when max-width is exceeded

    var distRight = window.outerWidth - ($miniCert.offset().left + $miniCert.width()); // Update positioning when scrolling is in fixed window and screen width is sufficient

    if (documentBottom > startFixed && documentBottom < endFixed && $(window).width() > 960) {
      $upgradeToVerifiedButton.addClass('attached');
      $upgradeToVerifiedButton.css('right', "".concat(distRight, "px"));
    } else {
      // If outside sliding window, reset to un-attached state
      $upgradeToVerifiedButton.removeClass('attached');
      $upgradeToVerifiedButton.css('right', '20px'); // Add class to define absolute location
  ...
  $toggleActionButton.on('click', function () {
    ...
    $verificationSock.slideToggle(toggleSpeed, fixUpgradeButton); // Toggle aria-expanded attribute

    var newAriaExpandedState = $toggleActionButton.attr('aria-expanded') === 'false';
    $toggleActionButton.attr('aria-expanded', newAriaExpandedState); // Log open and close events
