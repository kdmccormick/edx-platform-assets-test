...
/***/ "./common/static/xmodule/modules/js/002-7de2e4f56b8dbadb90e5de21d0fd31a5.js":
/***/ (function(module, exports, __webpack_require__) {
  ...
  (function () {
    ...
    this.Problem = function () {
      function Problem(element) {
        ...
        this.content = this.el.data('content'); // has_timed_out and has_response are used to ensure that
      ...
      Problem.prototype.bind = function () {
        ...
        this.submitNotification = this.$('.notification-submit'); // Accessibility helper for sighted keyboard users to show <clarification> tooltips on focus:
      ...
      Problem.prototype.renderProgressState = function () {
        ...
        graded = this.el.data('graded'); // The problem is ungraded if it's explicitly marked as such, or if the total possible score is 0
        ...
        if (curScore === undefined || totalScore === undefined) {
          ...
          if (graded) {
            progressTemplate = ngettext( // Translators: {num_points} is the number of points possible (examples: 1, 3, 10).;
          ...
          if (graded) {
            progressTemplate = ngettext( // This comment needs to be on one line to be properly scraped for the translators.
      ...
      Problem.prototype.poll = function (previousTimeout, focusCallback) {
        ...
        return $.postWithPrefix('' + this.url + '/problem_get', function (response) {
          var newTimeout; // If queueing status changed, then render
          ...
          if (that.num_queued_items === 0) {
            ...
            newTimeout = previousTimeout * 2; // if the timeout is greather than 1 minute
      ...
      Problem.prototype.submit_fd = function () {
        var abortSubmission,
            ...
            i,
            len,
            that = this; // If there are no file inputs in the problem, we can fall back on submit.
        ...
        fd = new FormData(); // Sanity checks on submission
        ...
        this.inputs.each(function (index, element) {
          ...
          if (element.type === 'file') {
            ...
            if (element.files.length === 0) {
              fileNotSelected = true; // In case we want to allow submissions with no file
        ...
        for (i = 0, len = errors.length; i < len; i++) {
          error = errors[i];
          errorHtml = __webpack_provided_edx_dot_HtmlUtils.joinHtml(errorHtml, __webpack_provided_edx_dot_HtmlUtils.interpolateHtml(__webpack_provided_edx_dot_HtmlUtils.HTML('<li>{error}</li>'), {
            error: error
          }));
        }

        errorHtml = __webpack_provided_edx_dot_HtmlUtils.interpolateHtml(__webpack_provided_edx_dot_HtmlUtils.HTML('<ul>{errors}</ul>'), {
          errors: errorHtml
        });
        ...
        return $.postWithPrefix('' + this.url + '/problem_check', this.answers, function (response) {
          switch (response.success) {
            ...
            case 'correct':
              ...
              that.updateProgress(response); // This is used by the Learning MFE to know when the Entrance Exam has been passed
        ...
        return $.postWithPrefix('' + this.url + '/problem_show', function (response) {
          ...
          $.each(answers, function (key, value) {
            ...
            if (!$.isArray(value)) {
              ...
              Collapsible.setCollapsibles(answer); // Sometimes, `value` is just a string containing a MathJax formula.
      ...
      Problem.prototype.refreshMath = function (event, element) {
        ...
        target = 'display_' + elid; // MathJax preprocessor is loaded by 'setupInputTypes'
      ...
      Problem.prototype.inputtypeShowAnswerMethods = {
        choicegroup: function choicegroup(element, display, answers, correctStatusHtml) {
          ...
          for (i = 0, len = answer.length; i < len; i++) {
            ...
            $inputStatus = $element.find('#status_' + inputId); // If the correct answer was already Submitted before "Show Answer" was selected,
          ...
          types = {
            ...
            regions: function regions(ctx, coords) {
              ...
              parseCoords = function parseCoords(coordinates) {
                var reg;
                reg = JSON.parse(coordinates); // Regions is list of lists [region1, region2, region3, ...] where regionN
...
(function () {
  ...
  var cktsim = function () {
    ...
    Circuit.prototype.load_netlist = function (netlist) {
      ...
      for (var i = netlist.length - 1; i >= 0; --i) {
        var component = netlist[i];
        var type = component[0]; // ignore wires, ground connections, scope probes and view info
        ...
        if (name == undefined || name == '') name = '_' + properties['_json_'].toString(); // convert node names to circuit indicies
    ...
    Circuit.prototype.find_solution = function (load, maxiters) {
      ...
      var down_count = 0;
      var thresh; // iteratively solve until values convere or iteration limit exceeded

      for (var iter = 0; iter < maxiters; iter++) {
        // set up equations
        load(this, soln, rhs); // Compute norm of rhs, assume variables of v type go with eqns of i type
        ...
        if (iter > 0 && use_limiting == false && abssum_old < abssum_rhs) {
          ...
          d_sol = mat_solve_rq(this.matrix, rhs); // If norm going down for ten iters, stop limiting
        ...
        if (iter == 0 || abssum_rhs > abssum_compare) abssum_compare = abssum_rhs; // Check residue convergence, but loosely, and give up
        // on last iteration

        if (iter < maxiters - 1 && abssum_rhs > res_check_abs + res_check_rel * abssum_compare) converged = false;else converged = true; // Update solution and check delta convergence
    ...
    Circuit.prototype.dc = function () {
      ...
      if (this.finalize() == false) return undefined; // Define -f and df/dx for Newton solver

      function load_dc(ckt, soln, rhs) {
        // rhs is initialized to -Gl * soln
        mat_v_mult(ckt.Gl, soln, rhs, -1.0); // G matrix is initialized with linear Gl

        mat_copy(ckt.Gl, ckt.G); // Now load up the nonlinear parts of rhs and G
      ...
      if (typeof iterations == 'undefined') {
        ...
        this.diddc = true; // create solution dictionary

        var result = []; // capture node voltages
    ...
    Circuit.prototype.tran = function (ntpts, tstart, tstop, probenames, no_dc) {
      ...
      function load_tran(ckt, soln, rhs) {
        ...
        mat_v_mult(ckt.Gl, soln, ckt.c, -1.0); // G matrix is initialized with linear Gl

        mat_copy(ckt.Gl, ckt.G); // Now load up the nonlinear parts of crnt and G
        ...
        mat_v_mult(ckt.C, soln, ckt.q, 1.0); // -rhs = c - dqdt
      ...
      var N = this.N; // build array to hold list of results for each variable
      ...
      this.beta1 = new Array(this.N); // Mark a set of algebraic variable (don't miss hidden ones!).

      this.ar = this.algebraic(this.C); // Non-algebraic variables and probe variables get lte
      ...
      this.time = tstart; // ntpts adjusted by numbers of periods in input

      this.max_step = (tstop - tstart) / (this.periods * ntpts);
      this.min_step = this.max_step / 1e8;
      var new_step = this.max_step / 1e6;
      this.oldt = this.time - new_step; // Initialize old crnts, charges, and solutions.
      ...
      var beta0, beta1; // Start with two pseudo-Euler steps, maximum 50000 steps/period
      ...
      for (var step_index = -3; step_index < max_nsteps; step_index++) {
        ...
        if (step_index < 0) {
          ...
          this.oldt = this.time; // Make sure we come smoothly in to the interval end.

          if (this.time >= tstop) break; // We're done.
          else if (this.time + new_step > tstop) this.time = tstop;else if (this.time + 1.5 * new_step > tstop) this.time += 2 / 3 * (tstop - this.time);else this.time += new_step; // Use trap (average old and new crnts.
        ...
        while (true) {
          ...
          this.alpha2 = 0; // If timestep is 1/10,000th of tstop, just use BE.
          ...
          var iterations = this.find_solution(load_tran, max_tran_iters); // If NR succeeds and stepsize is at min, accept and newstep=maxgrowth*minstep.
    ...
    Circuit.prototype.ac = function (npts, fstart, fstop, source_name) {
      ...
      var N = this.N;
      var G = this.G;
      var C = this.C; // Complex numbers, we're going to need a bigger boat

      var matrixac = mat_make(2 * N, 2 * N + 1); // Get the source used for ac
      ...
      this.device_map[source_name].load_ac(this, this.rhs); // build array to hold list of magnitude and phases for each node
      ...
      while (f <= fstop) {
        ...
        var solac = mat_solve(matrixac); // Save magnitude and phase

        for (var i = N - 1; i >= 0; --i) {
          var mag = Math.sqrt(solac[i] * solac[i] + solac[i + N] * solac[i + N]);
          response[i].push(mag); // Avoid wrapping phase, add or sub 180 for each jump

          var phase = 180 * (Math.atan2(solac[i + N], solac[i]) / Math.PI);
          var phasei = response[i + N];
          var L = phasei.length; // Look for a one-step jump greater than 90 degrees
    ...
    Circuit.prototype.opamp = function (np, nn, no, ng, A, name) {
      var ratio; // try to convert string value into numeric value, barf if we can't
    ...
    function mat_scale_add(A, B, scalea, scaleb, C) {
      ...
      } else if (_typeof(scaleb) instanceof Array && scalea instanceof Array) for (var i = 0; i < n; i++) {
        for (var j = 0; j < m; j++) {
    ...
    function mat_rank(Mo) {
      ...
      var temp, i, j; // Make a copy to avoid overwriting

      var M = mat_make(Nr, Nc);
      mat_copy(Mo, M); // Find matrix maximum entry
      ...
      for (var row = 0; row < Nr; row++) {
        ...
        for (var col = start_col; col < Nc; col++) {
          ...
          if (Math.abs(max_v) > eps * max_abs_entry) {
            start_col = col + 1;
            the_rank += 1; // Swap rows to get max in M[row][col]

            temp = M[row];
            M[row] = M[max_row];
            M[max_row] = temp; // now eliminate this column for all subsequent rows
    ...
    function mat_solve_rq(M, rhs) {
      ...
      for (var row = 0; row < Nr; row++) {
        ...
        if (row == 0) mat_scale = row_norm; // Check for all zero rows
    ...
    function mat_solve(M, rhs) {
      ...
      var temp, i, j; // Copy the rhs in to the last column of M if one is given.
      ...
      for (var col = 0; col < N; col++) {
        ...
        for (i = col + 1; i < N; i++) {
          ...
          if (temp != 0) // subtract current row from row we're working on
    ...
    function Device() {} // complete initial set up of device


    Device.prototype.finalize = function () {}; // Load the linear elements in to Gl and C


    Device.prototype.load_linear = function (ckt) {}; // load linear system equations for dc analysis
    // (inductors shorted and capacitors opened)


    Device.prototype.load_dc = function (ckt, soln, rhs) {}; // load linear system equations for tran analysis


    Device.prototype.load_tran = function (ckt, soln) {}; // load linear system equations for ac analysis:
    // current sources open, voltage sources shorted
    // linear models at operating point for everyone else


    Device.prototype.load_ac = function (ckt, rhs) {}; // return time of next breakpoint for the device
    ...
    function parse_number(s, default_v) {
      ...
      var multiplier = 1;
      var result = 0;
      var index = 0; // skip leading whitespace

      while (index < slen && s.charAt(index) <= ' ') {
        index += 1;
      }

      if (index == slen) return default_v; // check for leading sign
      ...
      if (index == start) return default_v; // type of multiplier determines type of result:
      ...
      result *= multiplier; // now check for exponent or engineering scale factor.  If there
    ...
    function parse_source(v) {
      ...
      if (index >= 0) {
        ...
        while (index < end) {
          ...
          if (arg_end == -1) arg_end = end; // parse and save result in our list of arg values
      ...
      // sin(v_offset,v_amplitude,freq_hz,t_delay,phase_offset_degrees)
        ...
        phase /= 360.0; // return value of source at time t
    ...
    function pwl_source(src, tv_pairs, repeat) {
      ...
      if (nvals <= 2) {
        ...
        src.value = function (t) {
          // closure
          if (repeat) // make time periodic if values are to be repeated
    ...
    VSource.prototype.constructor = VSource; // load linear part for source evaluation
    ...
    ISource.prototype.load_linear = function (ckt) {// Current source is open when off, no linear contribution
    }; // load linear system equations for dc analysis


    ISource.prototype.load_dc = function (ckt, soln, rhs) {
      var is = this.src.dc; // MNA stamp for independent current source
    ...
    ISource.prototype.load_tran = function (ckt, soln, rhs, time) {
      var is = this.src.value(time); // MNA stamp for independent current source
    ...
    Resistor.prototype.load_dc = function (ckt) {// Nothing to see here, move along.
    };
    ...
    Diode.prototype.load_linear = function (ckt) {// Diode is not linear, has no linear piece.
    };
    ...
    Diode.prototype.load_dc = function (ckt, soln, rhs) {
      ...
      var temp1, temp2; // Estimate exponential with a quadratic if arg too big.
      ...
      var gd = this.ais * (temp2 / this.vt); // MNA stamp for independent current source
    ...
    Inductor.prototype.load_dc = function (ckt, soln, rhs) {// Inductor is a short at dc, so is linear.
    };
    ...
    Opamp.prototype.load_dc = function (ckt, soln, rhs) {// Op-amp is linear.
    };
    ...
    Fet.prototype.load_linear = function (ckt) {// FET's are nonlinear, just like javascript progammers
    };
  ...
  schematic = function () {
    ...
    function Schematic(input) {
      ...
      if (parts == undefined || parts == 'None') {
        ...
        parts = [];
      } else parts = parts.split(','); // now add the parts to the parts bin
      ...
      if (parts.length == 0 && analyses.length == 0) this.diagram_only = true;else this.diagram_only = false; // see what we need to submit.  Expecting attribute of the form
      // submit_analyses="{'tran':[[node_name,t1,t2,t3],...],
      //                   'ac':[[node_name,f1,f2,...],...]}"

      var submit = input.getAttribute('submit_analyses');
      if (submit && submit.indexOf('{') != -1) this.submit_analyses = JSON.parse(submit);else this.submit_analyses = undefined; // toolbar
      ...
      this.zctl_top = this.sctl_y + this.sctl_r + 8; // repaint simply draws this buffer and then adds selected elements on top
      ...
      this.shiftKey = false;
      this.altKey = false;
      this.cmdKey = false; // make sure other code can find us!

      input.schematic = this;
      this.input = input; // set up DOM -- use nested tables to do the layout
      ...
      parts_table.cellSpacing = '0'; // fill in parts_table
      ...
      this.input.parentNode.insertBefore(toplevel, this.input.nextSibling); // process initial contents of diagram

      this.load_schematic(this.input.getAttribute('value'), this.input.getAttribute('initial_value')); // start by centering diagram on the screen
    ...
    Schematic.prototype.add_component = function (new_c) {
      this.components.push(new_c); // create undoable edit record here
    };
    ...
    Schematic.prototype.remove_connection_point = function (cp, old_location) {
      ...
      if (cplist) {
        ...
        if (index != -1) {
          cplist.splice(index, 1); // if no more connections at this location, remove
    ...
    Schematic.prototype.split_wire = function (w, cp) {
      ...
      w.remove(); // add two new wires with connection point cp in the middle
    ...
    Schematic.prototype.check_connection_points = function (w) {
      for (var locn in this.connection_points) {
        ...
        if (cplist && w.bisect_cp(cplist[0])) {
          ...
          this.redraw_background(); // stop here, new wires introduced by split will do their own checks
    ...
    Schematic.prototype.zoomall = function () {
      ...
      if (sch_w == 0 && sch_h == 0) {
        ...
        if (this.scale < zoom_min) this.scale = zoom_min;else if (this.scale > zoom_max) this.scale = zoom_max; // center the schematic
    ...
    Schematic.prototype.cut = function () {
      ...
      sch_clipboard = []; // look for selected components, move them to clipboard.
    ...
    Schematic.prototype.copy = function () {
      // clear previous contents
      sch_clipboard = []; // look for selected components, copy them to clipboard.
    ...
    Schematic.prototype.paste = function () {
      ...
      this.message('cursor ' + this.cursor_x + ',' + this.cursor_y); // clear current selections
    ...
    Schematic.prototype.load_schematic = function (value, initial_value) {
      ...
      if (value && value.indexOf('[') != -1) {
        ...
        var json = JSON.parse(value); // top level is a list of components
    ...
    Schematic.prototype.update_value = function () {
      ...
      this.label_connection_points(); // build JSON data structure, convert to string value for
      // input field
    ...
    Schematic.prototype.json = function () {
      var json = []; // output all the components/wires in the diagram
    ...
    Schematic.prototype.extract_circuit = function () {
      ...
      var netlist = this.json(); // since we've done the heavy lifting, update input field value
      // so user can grab diagram if they want

      this.input.value = JSON.stringify(netlist); // create a circuit from the netlist
    ...
    Schematic.prototype.dc_analysis = function () {
      ...
      if (ckt === null) return; // run the analysis
    ...
    Schematic.prototype.setup_ac_analysis = function () {
      ...
      this.dialog('AC Analysis', content, function (content) {
        var sch = content.sch; // retrieve parameters, remember for next time
    ...
    Schematic.prototype.ac_analysis = function (npts, fstart, fstop, ac_source_name) {
      ...
      if (typeof results == 'string') this.message(results);else {
        var x_values = results['_frequencies_']; // x axis will be a log scale
        ...
        if (this.submit_analyses != undefined) {
          ...
          if (submit != undefined) {
            // save a copy of the results for submission
            this.ac_results = {}; // save requested values for each requested node
            ...
            for (var j = 0; j < submit.length; j++) {
              ...
              var fvlist = []; // for each requested freq, interpolate response value

              for (var k = 1; k < flist.length; k++) {
                var f = flist[k];
                var v = interpolate(f, x_values, values); // convert to dB
        ...
        var probe_maxv = [];
        var probe_color = []; // Check for probe with near zero transfer function and warn
        ...
        for (var i = probes.length - 1; i >= 0; --i) {
          ...
          var v = results[label]; // convert values into dB relative to source amplitude
    ...
    Schematic.prototype.transient_analysis = function () {
      ...
      this.dialog('Transient Analysis', content, function (content) {
        ...
        if (ckt === null) return; // retrieve parameters, remember for next time

        sch.tran_tstop = content.fields[tstop_lbl].value; // gather a list of nodes that are being probed.  These
        ...
        if (typeof results == 'string') sch.message(results);else {
          if (sch.submit_analyses != undefined) {
            ...
            if (submit != undefined) {
              ...
              var times = results['_time_']; // save requested values for each requested node
              ...
              for (var j = 0; j < submit.length; j++) {
                ...
                var node = tlist[0];
                var values = results[node];
                var tvlist = []; // for each requested time, interpolate waveform value
          ...
          var x_legend = 'Time'; // set up plot values for each node with a probe
    ...
    Schematic.prototype.redraw_background = function () {
      ...
      c.lineCap = 'round'; // paint background color
    ...
    Schematic.prototype.redraw = function () {
      var c = this.canvas.getContext('2d'); // put static image in the background

      c.drawImage(this.bg_image, 0, 0); // selected components
      ...
      this.enable_tool('paste', sch_clipboard.length > 0); // connection points: draw one at each location
      ...
      if (!this.diagram_only) {
        var r = this.sctl_r;
        var x = this.sctl_x;
        var y = this.sctl_y; // circle with border
        ...
        c.beginPath();
        c.arc(x, y, r, 0, 2 * Math.PI);
        c.stroke(); // direction markers for scroll
        ...
        c.lineWidth = 1.0;
        c.beginPath(); // zoom in label

        c.moveTo(x + 4, y + 8);
        c.lineTo(x + 12, y + 8);
        c.moveTo(x + 8, y + 4);
        c.lineTo(x + 8, y + 12); // zoom out label

        c.moveTo(x + 4, y + 24);
        c.lineTo(x + 12, y + 24); // surround label
    ...
    try {
      if (HTMLCanvasElement) HTMLCanvasElement.prototype.relMouseCoords = function (event) {
        ...
        } while (currentElement = currentElement.offsetParent); // now compute relative position of click within the canvas
    ...
    function schematic_key_down(event) {
      ...
      var code = event.keyCode; // keep track of modifier key state

      if (code == 16) sch.shiftKey = true;else if (code == 17) sch.ctrlKey = true;else if (code == 18) sch.altKey = true;else if (code == 91) sch.cmdKey = true; // backspace or delete: delete selected components
        ...
        event.preventDefault();
        return false;
      } else return true; // consume keystroke
    ...
    function schematic_mouse_enter(event) {
      if (!event) event = window.event;
      var sch = window.event ? event.srcElement.schematic : event.target.schematic; // see if user has selected a new part

      if (sch.new_part) {
        // grab incoming part, turn off selection of parts bin
        var part = sch.new_part;
        sch.new_part = undefined;
        part.select(false); // unselect everything else in the schematic, add part and select it
        ...
        part.set_select(true); // and start dragging it
    ...
    function schematic_mouse_down(event) {
      ...
      var sch = window.event ? event.srcElement.schematic : event.target.schematic; // determine where event happened in schematic coordinates
      ...
      if (sx * sx + sy * sy <= sch.sctl_r * sch.sctl_r) {
        ...
        sch.cursor_y = Math.round(y / sch.grid) * sch.grid; // is mouse over a connection point?  If so, start dragging a wire
        ...
        if (cplist && !event.shiftKey) {
          ...
          if (!event.shiftKey) {
            ...
            // we just clicked on
            if (!reselect) sch.unselect_all(which); // if there's nothing to drag, set up a selection rectangle
    ...
    function schematic_mouse_move(event) {
      ...
      if (sch.wire) {
        ...
        if (dx != 0 || dy != 0) {
          ...
          sch.drag_y = sch.cursor_y; // give all components a shot at processing the event
    ...
    function schematic_mouse_up(event) {
      if (!event) event = window.event;else event.preventDefault();
      var sch = window.event ? event.srcElement.schematic : event.target.schematic; // drawing a new wire
      ...
      if (sch.dragging) sch.drag_end(); // selection rectangle

      if (sch.select_rect) {
        var r = sch.select_rect; // if select_rect is a point, we've already dealt with selection
        ...
        if (r[0] != r[2] || r[1] != r[3]) {
          ...
          canonicalize(s);
          if (!event.shiftKey) sch.unselect_all(); // select components that intersect selection rectangle
    ...
    function schematic_double_click(event) {
      ...
      var y = sch.canvas.mouse_y / sch.scale + sch.origin_y;
      sch.cursor_x = Math.round(x / sch.grid) * sch.grid;
      sch.cursor_y = Math.round(y / sch.grid) * sch.grid; // see if we double-clicked a component.  If so, edit it's properties
    ...
    Schematic.prototype.dialog = function (title, content, callback) {
      ...
      dialog.callback = callback; // look for property input fields in the content and give
      ...
      cancel_button.style.margin = '10px'; // div to hold the two buttons
      ...
      dialog.appendChild(buttons); // put into an overlay window
    ...
    function build_table(a) {
      var tbl = document.createElement('table'); // build a row for each element in associative array
    ...
    Schematic.prototype.window = function (title, content, offset) {
      ...
      win.draw_y = undefined; // div to hold the title
      ...
      win.appendChild(head); // capture mouse events in title bar

      head.addEventListener('mousedown', window_mouse_down, false); // div to hold the content
      //var body = document.createElement('div');
      //body.appendChild(content);
      ...
      win.top = this.canvas.mouse_y + offset; // add to DOM
    ...
    function bring_to_front(win, insert) {
      ...
      var i = wlist.indexOf(win); // remove from current position (if any) in window list

      if (i != -1) wlist.splice(i, 1); // if requested, add to end of window list

      if (insert) wlist.push(win); // adjust all zIndex values
    ...
    function window_close(win) {
      ...
      win.parentNode.removeChild(win); // remove from list of pop-up windows
    ...
    function window_mouse_down(event) {
      ...
      var src = window.event ? event.srcElement : event.target;
      var win = src.win;
      bring_to_front(win, true); // add handlers to document so we capture them no matter what

      document.addEventListener('mousemove', window_mouse_move, false);
      document.addEventListener('mouseup', window_mouse_up, false);
      document.tracking_window = win; // remember where mouse is so we can compute dx,dy during drag
    ...
    function window_mouse_up(event) {
      var win = document.tracking_window; // show's over folks...
    ...
    function window_mouse_move(event) {
      ...
      if (win.drag_x) {
        ...
        var dy = event.pageY - win.drag_y; // move the window

        win.left += dx;
        win.top += dy;
        win.style.left = win.left + 'px';
        win.style.top = win.top + 'px'; // update reference point
    ...
    Schematic.prototype.add_tool = function (icon, tip, callback) {
      ...
      tool.style.verticalAlign = 'top'; // set up event processing

      tool.addEventListener('mouseover', tool_enter, false);
      tool.addEventListener('mouseout', tool_leave, false);
      tool.addEventListener('click', tool_click, false); // add to toolbar
    ...
    Schematic.prototype.enable_tool = function (tname, which) {
      ...
      if (tool != undefined) {
        ...
        tool.enabled = which; // if disabling tool, remove border and tip
    ...
    function engineering_notation(n, nplaces, trim) {
      ...
      var mantissa = sign * Math.pow(10, log10 - 3 * exp); // keep specified number of places following decimal point
    ...
    var cursor_pattern = [5, 5]; // x_values is an array of x coordinates for each of the plots
    ...
    Schematic.prototype.graph = function (x_values, x_legend, y_values, y_legend, z_values, z_legend) {
      ...
      canvas.width = w;
      canvas.height = h; // the graph itself will be drawn here and this image will be copied
      ...
      c.fillRect(0, 0, w, h);
      c.fillStyle = element_style;
      c.fillRect(left_margin, top_margin, pwidth, pheight); // figure out scaling for plots
      ...
      canvas.sch = this; // do something useful when user mouses over graph

      canvas.addEventListener('mousemove', graph_mouse_move, false); // return our masterpiece
    ...
    function plot_cursor(c, graph, cursor_x, left_margin) {
      ...
      c.stroke(); // add x label at bottom of marker
      ...
      c.fillText("\u2588\u2588\u2588\u2588\u2588", x, end_y);
      c.fillStyle = normal_style;
      c.fillText(engineering_notation(graph_x, 3, false), x, end_y); // compute which points marker is between
      ...
      if (x2 != undefined) {
        ...
        if (graph.y_values != undefined) {
          for (var plot = 0; plot < graph.y_values.length; plot++) {
            ...
            var y2 = values[index];
            var y = y1;
            if (graph_x != x1) y += (graph_x - x1) * (y2 - y1) / (x2 - x1); // annotate plot with value of signal at marker
        ...
        if (graph.z_values != undefined) {
          ...
          for (var plot = 0; plot < graph.z_values.length; plot++) {
            ...
            var z2 = values[index];
            var z = z1;
            if (graph_x != x1) z += (graph_x - x1) * (z2 - z1) / (x2 - x1); // annotate plot with value of signal at marker
    ...
    function graph_mouse_move(event) {
      ...
      g.relMouseCoords(event); // not sure yet where the 3,-3 offset correction comes from (borders? padding?)
    ...
    function Part(sch) {
      ...
      this.selected = false; // set up canvas

      this.canvas = document.createElement('canvas');
      this.canvas.style.borderStyle = 'solid';
      this.canvas.style.borderWidth = '1px';
      this.canvas.style.borderColor = background_style; //this.canvas.style.position = 'absolute';
      ...
      this.canvas.addEventListener('mouseup', part_mouse_up, false); // make the part "clickable" by registering a dummy click handler
    ...
    Part.prototype.set_component = function (component, tip) {
      ...
      this.tip = tip; // figure out scaling and centering of parts icon
    ...
    Part.prototype.redraw = function (part) {
      var c = this.canvas.getContext('2d'); // paint background color
    ...
    Part.prototype.update_connection_point = function (cp, old_location) {// no connection points in the parts bin
    };
    ...
    Part.prototype.draw_text = function (c, text, x, y, size) {// no text displayed for the parts icon
    };

    function part_enter(event) {
      if (!event) event = window.event;
      var canvas = window.event ? event.srcElement : event.target;
      var part = canvas.xpart; // avoid Chrome bug that changes to text cursor whenever
    ...
    function part_leave(event) {
      ...
      if (typeof part.sch.new_part == 'undefined') {// leaving with no part selected?  revert handler
    ...
    function canonicalize(r) {
      var temp; // canonicalize bounding box
    ...
    function intersect(r1, r2) {
      ...
      var result = !(r2[0] > r1[2] || r2[2] < r1[0] || r2[1] > r1[3] || r2[3] < r1[1]); // if I try to return the above expression, javascript returns undefined!!!
    ...
    Component.prototype.update_coords = function () {
      ...
      canonicalize(this.bbox); // update connections
    ...
    Component.prototype.rotate = function (amount) {
      ...
      this.update_coords(); // create an undoable edit record here
      // using old_rotation
    };
    ...
    Component.prototype.remove = function () {
      ...
      this.sch = undefined; // create an undoable edit record here
    };
    ...
    Component.prototype.set_select = function (which) {
      if (which != this.selected) {
        this.selected = which; // create an undoable edit record here
      }
    };
    ...
    Component.prototype.propagate_label = function (label) {}; // give components a chance to generate default labels for their connection(s)
    // default action: do nothing


    Component.prototype.add_default_labels = function () {}; // component should generate labels for all unlabeled connections
    ...
    ConnectionPoint.prototype.propagate_label = function (label) {
      ...
      if (this.label === undefined) {
        ...
        this.label = label; // propagate label to coincident connection points

        this.parent.sch.propagate_label(label, this.location); // possibly label other cp's for this device?
    ...
    ConnectionPoint.prototype.update_location = function () {
      ...
      this.x = nx;
      this.y = ny;
      this.location = nx + ',' + ny; // add ourselves to the connection list for the new location
    ...
    ConnectionPoint.prototype.display_voltage = function (c, vmap) {
      ...
      if (v != undefined) {
        var label = v.toFixed(2) + 'V'; // first draw some solid blocks in the background

        c.globalAlpha = 0.85;
        this.parent.draw_text(c, "\u2588\u2588\u2588", this.offset_x, this.offset_y, 4, annotation_size, element_style);
        c.globalAlpha = 1.0; // display the node voltage at this connection point

        this.parent.draw_text(c, label, this.offset_x, this.offset_y, 4, annotation_size, annotation_style); // only display each node voltage once
    ...
    function Wire(x1, y1, x2, y2) {
      ...
      this.add_connection(this.dx, this.dy); // compute bounding box (expanded slightly)
    ...
    Wire.prototype.bisect_cp = function (cp) {
      var x = cp.x;
      var y = cp.y; // crude check: (x,y) within expanded bounding box of wire

      if (inside(this.bbox, x, y)) {
        // compute distance between x,y and nearst point on line
        // http://www.allegro.cc/forums/thread/589720
        var D = Math.abs((x - this.x) * this.dy - (y - this.y) * this.dx) / this.len; // final check: ensure point isn't an end point of the wire
    ...
    Wire.prototype.move_end = function () {
      ...
      this.sch.check_wires(this); // look for connection points that might bisect us
    ...
    Probe.prototype.draw = function (c) {
      ...
      this.draw_arc(c, 19, -11, 8, 3 * Math.PI / 2, 0); // fill body with plot color
    ...
    Ammeter.prototype.draw = function (c) {
      this.draw_line(c, 0, 0, 16, 0); // draw chevron in probe color
    ...
    Ammeter.prototype.display_current = function (c, vmap) {
      ...
      if (v != undefined) {
        ...
        this.draw_text(c, i, 8, -5, 7, annotation_size, annotation_style); // only display each current once
    ...
    function source_type_changed(event) {
      if (!event) event = window.event;
      var select = window.event ? event.srcElement : event.target; // see where to get source parameters from
    ...
    VSource.prototype.edit_properties = Source.prototype.edit_properties; // display current for DC analysis
    ...
    VSource.prototype.display_current = function (c, vmap) {
      ...
      if (v != undefined) {
        ...
        c.globalAlpha = 1.0; // display the element current

        var i = engineering_notation(v, 2) + 'A';
        this.draw_text(c, i, -3, 5, 5, annotation_size, annotation_style); // draw arrow for current

        this.draw_line(c, -3, 4, 0, 8);
        this.draw_line(c, 3, 4, 0, 8); // only display each current once
    ...
    function component_slider(event, ui) {
      var sname = $(this).slider("option", "schematic"); // set value of specified component
      ...
      if (choices instanceof Array) v = choices[v]; // selector may match several schematics
...
__webpack_require__("./common/static/xmodule/modules/js/002-7de2e4f56b8dbadb90e5de21d0fd31a5.js");
